      ...
      <div className={`flex gap-4 ${cls}`} style={{ width: 'max-content' }}>
        {doubled.map((item, i) => (
          ...
          >
            ...
            {/* Label */}
            <figcaption className="absolute inset-x-0 bottom-0 translate-y-2 p-4 text-[10px] uppercase tracking-[0.28em] text-white/90 opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
              {item.label}
            </figcaption>
          </figure>
        ))}
  ...
  return (
    <section className="section-shell pt-8 pb-0">
      ...
      <div aria-hidden="true" className="marquee-outer relative overflow-hidden">
        {/* Edge fade masks */}
        <div className="pointer-events-none absolute left-0 top-0 z-10 h-full w-32 bg-gradient-to-r from-ink to-transparent" />
        <div className="pointer-events-none absolute right-0 top-0 z-10 h-full w-32 bg-gradient-to-l from-ink to-transparent" />
